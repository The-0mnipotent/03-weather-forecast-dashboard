...
import {
  CompleteTodayWeatherData,
  todayWeatherJsonToObject,
  WeeklyForecast,
  weeklyWeatherJsonToObject,
} from "./utils/WeatherApiFunctions";
...
function App() {
  ...
  const [todayForecast, setTodayForecast] =
    useState<CompleteTodayWeatherData | null>(null);
  const [weeklyForecast, setWeeklyForecast] = useState<WeeklyForecast[] | null>(
    null
  );
  ...
  const onSearch = async (option: Option) => {
    ...
    setIsLoading(true);
    try {
      const [todayForecastResponse, weeklyForecastResponse] =
        await weatherApiService.fetchWeatherData(latitude, longitude);
      ...
      const completeTodayWeatherData: CompleteTodayWeatherData =
        todayWeatherJsonToObject(todayForecastResponse, weeklyForecastResponse);
      const weeklyWeatherData: WeeklyForecast[] = weeklyWeatherJsonToObject(
